...
pub fn contain(sources: &String, value: &Vec<&str>) -> bool {
    let bot_name = CONFIG_CONTEXT.bot_config.bot_name.as_ref().unwrap();
    let bot_id = CONFIG_CONTEXT.bot_config.bot_id.as_ref().unwrap();
    let mut vec1 = vec![];
    for msg in value {
        let string = msg
...
pub fn rex_utils(select:i8, msg: &String, name: Option<String>, regex: &str, qq: Option<i64>,no_message:Vec<&str>) -> (bool, String) {
    let bot_name = CONFIG_CONTEXT.bot_config.bot_name.as_ref().unwrap().clone();
    let bot_id = CONFIG_CONTEXT.bot_config.bot_id.as_ref().unwrap();
